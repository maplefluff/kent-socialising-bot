...
export class GuildBanRemoveListener extends Listener {
	public async run(ban: GuildBan) {
		try {
			...
			const threadChannel = await this.container.utilities.modlogUtilities.fetchThreadChannel('MEMBERS');
			...
			return threadChannel.send({
				embeds: [
					new EmbedBuilder()
						...
						.setTitle(`Member unbanned by ${executor?.username || 'Unknown moderator'}`)
						.addFields(
							{
