...
export class WarningsSubcommand extends Subcommand {
	...
	public override async autocompleteRun(
		...
		const guildWarnings = await this.container.prisma.warning.findMany({
			where: isNullishOrEmpty(idToMatch)
				? {}
				: { id: Number.parseInt(idToMatch.toString(), 10) },
		...
		if (!guildWarnings)
			return interaction.respond([
				{ value: 0, name: "There are no warnings to display" },
			]);
	...
	public async removeWarning(
		interaction: Subcommand.ChatInputCommandInteraction,
	) {
		const warningId = interaction.options.getInteger("warning_id");
		if (!warningId)
			return interaction.reply("Please provide a valid warning ID to remove");

		try {
			const warning = await this.container.prisma.warning.delete({
				where: { id: warningId },
			});
			...
			return interaction.reply(
				`Warning ${warningId}, assigned to <@${warning.userId}> by <@${warning.moderatorId}> has been removed`,
			);
		} catch (error) {
			this.container.logger.error(error);
			return interaction.reply(
				"An error occured while trying to remove the warning",
			);
		}
	}
	...
	public async clearWarnings(
		...
		if (!member)
			return interaction.reply(
				"Please provide a valid user to clear warnings for",
			);

		try {
			await this.container.prisma.warning.deleteMany({
				where: { userId: BigInt(member.id) },
			});
			...
			return interaction.reply(
				`All warnings for <@${member.id}> have been cleared`,
			);
		} catch (error) {
			this.container.logger.error(error);
			return interaction.reply(
